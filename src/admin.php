...
function makePostTable($accessCounter, $post) {
  ...
  <tr>
    ...
    <td class="table-center">' . $hidden . '</td>
    <td>' . $post['title'] . '</td>
...
if ( @$_GET['no'] && preg_match('/^[0-9]{1,16}$/', @$_GET['no']) ) {
  ...
  <div class="table-wrap">
    <table class="admin__table">
      <thead>
        <tr>
          <th>編集</th><th>No.</th><th>状態</th><th>タイトル</th><th>説明</th><th>アクセス数</th>
        </tr>
      </thead>
      <tbody>
  ';
  if ( $postData ) foreach( $postData as $post ) makePostTable($accessCounter, $post);
  echo '
      </tbody>
    </table>
  </div>
