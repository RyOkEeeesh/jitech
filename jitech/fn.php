...
WITH nos AS (
  SELECT
    no,
    LAG(no) OVER (ORDER BY no DESC) previous_no,
    LEAD(no) OVER (ORDER BY no DESC) next_no
  FROM
    overview
  WHERE
...
combined AS (
  SELECT
    nos.no,
    COALESCE(nos.previous_no, first_last.last_no) previous_no,
    COALESCE(nos.next_no, first_last.first_no) next_no
  FROM
    nos, first_last
)
SELECT 
